...
alone: jar clean-local-output
	${hadoop.root}/bin/hadoop jar ${jar.path} ${job.name} ${local.input} ${local.output} ${args.alpha} ${args.noOfIteration} ${args.kForTopK} 
...
cloud: jar upload-app-aws delete-output-aws
	aws emr create-cluster \
	    ...
	    --steps '[{"Args":["${job.name}","s3://${aws.bucket.name}/${aws.input}","s3://${aws.bucket.name}/${aws.output}","${args.alpha}","${args.noOfIteration}","${args.kForTopK}"],"Type":"CUSTOM_JAR","Jar":"s3://${aws.bucket.name}/${jar.name}","ActionOnFailure":"TERMINATE_CLUSTER","Name":"Custom JAR"}]' \
